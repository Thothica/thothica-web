...
import { type indexSelection } from "@/app/search/page";

const Searchbar = ({ tags }: { tags: indexSelection[] }) => {
    ...
    const handleSearch = () => {
        ...
        if (!searchQuery || !selectedTag) {
            toast({
                ...
                description: "No tag selected or your query is empty"
            })
            return;
        }
            ...
            <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        className={`rounded-full border-muted-foreground shadow-md hover:bg-primary hover:text-primary-foreground ${selectedTag && selectedTag.id === tag.id
