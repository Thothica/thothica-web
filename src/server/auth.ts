...
export const authOptions: NextAuthOptions = {
    ...
    events: {
        async createUser(message) {
            ...
            const userInformation = await db.query.allowedEmails.findFirst({
                where: (fields, { eq }) => eq(fields.email, userEmail),
            });
            if (!userInformation) return

            if (userInformation.role == "tos") {
                await db.update(users).set({ userRole: "tos" }).where(eq(users.email, userEmail))
            }
        },
    },
