...
const userTiers = ["thothica_generation", "tos"] as const
export type Role = (typeof userTiers)[number]
...
export const users = createTable("user", {
    ...
    userRole: text("user_role", { enum: userTiers }).notNull().default("thothica_generation")
});
...
export const allowedEmails = createTable(
    "allowedEmail",
    {
        ...
        name: text("name", { length: 255 }),
        email: text("email", { length: 255 }).notNull(),
        role: text("role", { enum: userTiers }).notNull().default("thothica_generation")
    },
    (table) => ({
